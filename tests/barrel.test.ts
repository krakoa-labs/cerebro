...
describe("parseBarrel", () => {
  ...
  it("extracts a local variable export", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Tooltip", source: null, importedName: null, shape: "barrel-local" },
    ]);
    expect(result.warnings).toEqual([]);
  });
  ...
  it("extracts a local function export", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Banner", source: null, importedName: null, shape: "barrel-local" },
    ]);
  });
  ...
  it("extracts a local class export", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Widget", source: null, importedName: null, shape: "barrel-local" },
    ]);
  });
  ...
  it("extracts a local TypeScript type export", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Variant", source: null, importedName: null, shape: "barrel-local" },
    ]);
  });
  ...
  it("extracts a named re-export with the original name as importedName", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Button", source: "./Button", importedName: "Button", shape: "named-reexport" },
    ]);
  });
  ...
  it("filters 'default' from importedName on a default-as-named re-export", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Card", source: "./Card", importedName: null, shape: "default-reexport" },
    ]);
  });
  ...
  it("keeps the source-module name as importedName on a renamed re-export", () => {
    ...
    expect(result.exports).toEqual([
      { name: "Dialog", source: "./Modal", importedName: "Modal", shape: "renamed-reexport" },
    ]);
  });
  ...
  it("warns on a namespace re-export without producing an entry", () => {
    const result = parseBarrel(`export * as Forms from "./Forms";`, "index.ts");
    expect(result.exports).toEqual([]);
    expect(result.warnings).toEqual([{ code: "namespace-reexport", detail: "./Forms" }]);
  });
    ...
    expect(result.exports).toEqual([
      {
        name: "FancySelect",
        source: "./FancySelect",
        importedName: "FancySelect",
        shape: "named-reexport",
      },
      {
        name: "FancyAsyncSelect",
        source: "./FancySelect",
        importedName: "FancyAsyncSelect",
        shape: "named-reexport",
      },
    ]);
  });
