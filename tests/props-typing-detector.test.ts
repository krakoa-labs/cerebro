...
describe("detectPropsTyping — typed (named lookup)", () => {
  it.each([
    ...
    expect(detectPropsTyping(src, "Button.tsx", named)).toBe("typed");
  });

  it("treats a function component that accepts no props as typed", () => {
    expect(detectPropsTyping("export function Button() {}", "Button.tsx", named)).toBe("typed");
    expect(detectPropsTyping("export const Button = () => null;", "Button.tsx", named)).toBe(
      "typed",
    );
  });
...
describe("detectPropsTyping — untyped (named lookup)", () => {
  it.each([
    ...
    expect(detectPropsTyping(src, "Button.tsx", named)).toBe("untyped");
  });
});
...
describe("detectPropsTyping — unanalyzed (named lookup)", () => {
  it("classifies a class component as unanalyzed", () => {
    ...
    expect(detectPropsTyping(src, "Button.tsx", named)).toBe("unanalyzed");
  });
  ...
  it("returns unanalyzed when no declaration matches the lookup name", () => {
    ...
    expect(detectPropsTyping(src, "Card.tsx", named)).toBe("unanalyzed");
  });

  it("returns unanalyzed for a bare re-export from another module", () => {
    const src = `export { Button } from "./impl";`;
    expect(detectPropsTyping(src, "index.tsx", named)).toBe("unanalyzed");
  });

  it("returns unanalyzed for an empty file", () => {
    expect(detectPropsTyping("", "Button.tsx", named)).toBe("unanalyzed");
  });
});
...
describe("detectPropsTyping — default lookup", () => {
  ...
  it("classifies a typed default-exported function declaration as typed", () => {
    ...
    expect(detectPropsTyping(src, "Button.tsx", dflt)).toBe("typed");
  });
  ...
  it("classifies an untyped anonymous default-exported arrow as untyped", () => {
    expect(detectPropsTyping("export default (props) => null;", "Button.tsx", dflt)).toBe(
      "untyped",
    );
  });
  ...
  it("classifies a default-exported class component as unanalyzed", () => {
    ...
    expect(detectPropsTyping(src, "Button.tsx", dflt)).toBe("unanalyzed");
  });
